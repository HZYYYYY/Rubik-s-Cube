        ...
        if len(cent) >= 5:
            ...
            if len(cents)>4 and validate(cents)<10:
                if vision_params.face_col[4] == last_face:
                    ...
                    if abs(move[0]) >= abs(move[1]) and move[0] < 0 :
                        text = "Right move"
                    if abs(move[0]) >= abs(move[1]) and move[0] > 0 :
                        text = "Left move"
                    if abs(move[1]) >= abs(move[0]) and move[1] < 0 :
                        text = "Up move"
                    if abs(move[1]) >= abs(move[0]) and move[1] > 0 :
                        text = "Down move"

                    cv2.putText(bgrcap, text, (500, 100), cv2.FONT_HERSHEY_PLAIN, 4.0, (0, 0, 255), 2)
...
#         text = "Right"
#         cv2.putText(bgrcap, text, (500, 50), cv2.FONT_HERSHEY_PLAIN, 4.0, (0, 0, 255), 2)
...
#         vision_params.face_hsv, vision_params.face_col = getcolors(cf, ef, acf, aef, m)
        ...
        if flag:   
            solution,success = solve(config_all)
            if success>1:
                print('oh my god', success)
            #print(success)
            if len(solutions) == 0 and success>0:
                solutions.append(solution)
                text = "Solution found! First move is {}".format(solution.split(' ')[0])
            ...
            elif success>0 and len(solutions) >0:
                if solution.split(' ') == solutions[-1].split(' ')[1:]:
                    text ="Right move! Next move is: {}".format(solution.split(' ')[0])
                    print('right', solution)
                else:
                    text ="Wrong move! Next move is: {}".format(solution.split(' ')[0])
